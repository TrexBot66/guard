...
mongoose.connect('mongodb+srv://emirhankiymaz66:e.k200666@cluster0.nlf3k.mongodb.net/myFirstDatabase?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});
mongoose.connection.on("open", async() => {
console.log("Mongo Bağlandı.")
})

client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "HydroniX 💛 1740" }, status: "idle" });
...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("ORANGE").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter("HydroniX").setTimestamp();
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "HydroniX Guard Tarafından Banlandı." }).catch();
};
...
client.on("channelCreate", async channel => {
  ...
  channel.delete({reason: "HydroniX Kanal Koruması"});
  cezalandir(entry.executor.id, "ban");
  let logKanali = client.channels.cache.get(ayarlar.logChannelID);
...
client.on("channelDelete", async channel => {
  ...
  if (!entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id) || !ayarlar.channelGuard) return;
  await channel.clone({ reason: "HydroniX Kanal Koruma" }).then(async kanal => {
...
if (!entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id) || !ayarlar.channelGuard) return;
await role.delete({ reason: "HydroniX Rol Koruması" });
